...
export class CombatSystem {
    ...
    constructor(game) {
        ...
        // ── Mano y decks ──────────────────────────────────────────────────────
        // Jugador: usa el deckList del game (ciclo Clash Royale)
        this.playerHand  = []
        this.playerCycle = []   // cola de ciclo (copia barajada del deckList)

        // Enemigo: mantiene su propio sistema
        this.enemyHand = []
        this.enemyDeck = []
    ...
    _drawCard(side) {
        if (side === "player") {
            ...
            if (this.enemyHand.length >= 7) return
    ...
    // ── Jugar carta: devolverla al ciclo ──────────────────────────────────────
    _playCardFromHand(index) {
        const card = this.playerHand[index]
        if (!card) return null
        this.playerHand.splice(index, 1)
        // Devolver al final del ciclo
    ...
    placeCardInSlot(row, col) {
        ...
        // Devolver carta al ciclo (rotación Clash Royale)
    ...
    attackTarget(row, col) {
        ...
        // Sacerdote Oscuro: cura al protector al atacar
    ...
    // ── Sacerdote Oscuro: cura 2 HP a la tropa melee que la protege ──────────
    ...
    _checkGameOver() {
        if (this.enemyHealth <= 0) {
            this.gameOver = true
            // Guardar HP del jugador para el siguiente nivel
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // VISOR DE MAZO — 10 slots fijos, rotación visible
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    _renderDeckViewer() {
        ...
        // Construir los 10 slots
        const slots = []
        for (let i = 0; i < 10; i++) {
            slots.push(deckList[i] || null)
        }

        // Posición en el ciclo de cada carta
        const cycleMap = {}
    ...
    _renderCard(card, index) {
        ...
        "
            ...
            <div class="card-body">
                ...
                <div class="card-type">${isSpell ? "✨ Hechizo" : card.subtype === "ranged" ? "🏹 Distancia" : "⚔️ Melee"}</div>
                ${!isSpell ? `
