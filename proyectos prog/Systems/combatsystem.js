...
export class CombatSystem {
    ...
    constructor(game) {
        ...
        // MEJORA 1: Set de IDs de cartas ya usadas en la run (no vuelven a salir)
        // Se preserva entre niveles usando game.usedCardIds
        this.usedCardIds = game.usedCardIds || new Set()

        // MEJORA 2: Set de IDs en mano actualmente (para el visor de mazo)
        // y lista completa de cartas del mazo del jugador (todas las disponibles)
        this.allPlayerCards = [] // todas las cartas que pueden salir en la run
    ...
    _buildDeck(side) {
        if (side === "player") {
            // MEJORA 1: excluir cartas ya usadas en runs anteriores de este nivel
            const all = Object.values(cardsData)
            this.allPlayerCards = [...all] // guardar referencia completa para el visor
            const available = all.filter(c => !this.usedCardIds.has(c.id))
            // Si ya se usaron todas, resetear (nueva run virtual)
            const pool = available.length >= 5 ? available : [...all]
    ...
    _drawCard(side) {
        if (side === "player") {
            ...
            if (this.playerHand.length < 5) {
                ...
                // MEJORA 1: registrar carta como "vista/en mano" en usedCardIds
                this.usedCardIds.add(card.id)
                // Sincronizar con el game para persistir entre niveles
                if (this.game) this.game.usedCardIds = this.usedCardIds
            }
        } else {
    ...
    // ─── Guardián del Abismo interactivo ─────────────────────────────────────
    ...
    // MEJORA 2: Toggle visor de mazo
    toggleDeckViewer() {
        this.showDeckViewer = !this.showDeckViewer
        this.render()
    }
    ...
    render() {
        ...
        // MEJORA 2: calcular estado de cada carta para el visor de mazo
        const handIds  = new Set(this.playerHand.map(c => c.id))
        const deckIds  = new Set(this.playerDeck.map(c => c.id))
        ...
        <!-- MEJORA 2: Visor de mazo -->
        ${this.showDeckViewer ? this._renderDeckViewer(handIds, deckIds) : ""}

        <!-- Preview modal -->
        ${this.previewCard ? this._renderPreviewModal(this.previewCard) : ""}

        <!-- MEJORA 3: Game Over / Victoria — sin botón reiniciar en victoria -->
        ${this.gameOver ? `
        ...
        if (deckOverlay) {
            deckOverlay.onclick = (e) => {
                if (e.target === deckOverlay) { this.showDeckViewer = false; this.render() }
            }
        }
    }
    ...
    _renderDeckViewer(handIds, deckIds) {
        const allCards = this.allPlayerCards.length > 0
            ? this.allPlayerCards
            : Object.values(cardsData)

        // Separar en disponibles (en mazo o mano) y usadas (jugadas / descartadas)
        const available = allCards.filter(c => deckIds.has(c.id) || handIds.has(c.id))
        const used      = allCards.filter(c => !deckIds.has(c.id) && !handIds.has(c.id))

        const renderCardThumb = (card, dimmed) => {
            ...
            return `
            <div class="dv-card ${dimmed ? 'dv-card-used' : 'dv-card-available'} ${inHand ? 'dv-card-inhand' : ''}"
                title="${card.name}${inHand ? ' (en mano)' : dimmed ? ' (ya jugada)' : ' (en mazo)'}">
                ...
                ${dimmed ? '<div class="dv-badge dv-badge-used">USADA</div>' : ''}
            </div>`
        }
        ...
        <div class="deck-viewer-overlay" id="deck-viewer-overlay">
            <div class="deck-viewer-modal">
                <div class="dv-header">
                    <span class="dv-title">🂠 VISOR DE MAZO</span>
                    <div class="dv-legend">
                        <span class="dv-legend-item"><span class="dv-dot dv-dot-available"></span> En mazo (${available.length})</span>
                        <span class="dv-legend-item"><span class="dv-dot dv-dot-hand"></span> En mano (${handIds.size})</span>
                        <span class="dv-legend-item"><span class="dv-dot dv-dot-used"></span> Jugadas (${used.length})</span>
                    </div>
                    <button class="dv-close" onclick="window.game.combatSystem.toggleDeckViewer()">✕</button>
                </div>
                <div class="dv-grid">
                    ${available.map(c => renderCardThumb(c, false)).join("")}
                    ${used.map(c => renderCardThumb(c, true)).join("")}
                </div>
            </div>
        </div>`
    }
